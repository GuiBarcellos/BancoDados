...
/*
Seleciona a media do ano de nascimento das turmas, 
pode auxiliar na alocacao de um aluno para as turmas
*/
SELECT CODIGO, N_ALUNOS, AVG(ANO_NASC) AS MEDIA FROM TURMA T, COMPOE C, ALUNO A
...
TESTAR
*/
SELECT C.TURMA, A.GRADUACAO, COUNT(A.GRADUACAO) FROM ALUNO A
	JOIN COMPOE C ON A.CPF = C.ALUNO
	JOIN PESSOA P ON A.CPF = P.CPF
	GROUP BY C.TURMA, A.GRADUACAO
	ORDER BY C.TURMA;
...
*/
SELECT ALUNO.CPF, PESSOA.NOME FROM ALUNO, PESSOA
WHERE NOT EXISTS 
((SELECT CODIGO FROM TURMA)
EXCEPT
(SELECT TURMA FROM COMPOE
WHERE ALUNO.CPF = COMPOE.ALUNO)) AND PESSOA.CPF = ALUNO.CPF
